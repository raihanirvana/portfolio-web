...
function ModalContent({ content, onClose, openKey }: ModalProps) {
  ...
  return (
    ...
    >
      <ModalShell onClose={onClose} title={selected.title} subtitle={selected.subtitle} closeLabel={content.modalCloseLabel}>
        {selected.content}
      </ModalShell>
    </m.div>
  );
...
function ModalHeaderTitle({ subtitle, title }: { subtitle: string; title: string }) {
  return (
    <div>
      ...
      <h2 className="mt-2 text-2xl font-semibold tracking-tight text-zinc-950 sm:text-3xl" id="modal-title">
        {title}
      </h2>
    </div>
  );
